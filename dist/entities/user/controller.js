...
export const userList = async (req) => {
    ...
    if (req.token.role != "admin") {
        return User.find({ _id: req.token.id }, { updated_at: 0, password: 0, deleted_at: 0, created_at: 0 });
    }
    else {
        return User.find({
            $or: [
...
export const userListByID = async (req) => {
    if (req.token.role == "client" && req.params.id == req.token.id) {
        return User.findById({ _id: req.params.id }, { updated_at: 0, password: 0, deleted_at: 0, created_at: 0 });
    }
...
export const deleteteUser = async (data) => {
    const user = User.findOne({ _id: data.params.id, deleted_at: null });
    if (!user) {
